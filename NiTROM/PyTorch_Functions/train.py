    ...
    *,
    ...
    for epoch in range(num_epochs):
        ...
        def _apply_qr_update(phi, psi, gphi, gpsi, lr):
            ...
            param_change = torch.norm(phi - phi_old)/(r**0.5) + torch.norm(psi - psi_old)/(r**0.5)
        ...
        def _compute_and_set_grads():
            ...
            if is_nitrom and manifold_retraction == "qr":
                gphi, gpsi = _project_manifold_grads(params.Phi, params.Psi, grad_Phi, grad_Psi)
        ...
        if is_lbfgs:
            ...
            lr_manifold_used = manifold_lr if manifold_lr is not None else lr_tensors_used

            if use_line_search and not is_lbfgs:
                params = model.params

                lr_tensors = optimizer.param_groups[0]["lr"]
                lr_manifold = manifold_lr if manifold_lr is not None else optimizer.param_groups[0]["lr"]
                if (not _ls_user_set_alpha_init) and hasattr(line_searcher, "min_alpha"):
                ...
                if is_nitrom and manifold_retraction == "qr":
                    if last_gphi is None or last_gpsi is None:
                        raise RuntimeError("Expected manifold gradients to be computed for line search.")
                    param_list.extend([params.Phi, params.Psi])
                    direction_list.extend([lr_manifold * last_gphi, lr_manifold * last_gpsi])
                ...
                with torch.no_grad():
                    accepted = [p - alpha * d for p, d in zip(param_list, direction_list)]
                    if is_nitrom and manifold_retraction == "qr":
                ...
                if safe_step:
                    ...
                    for attempt in range(tries):
                        ...
                        if is_nitrom and manifold_retraction == "qr":
                            if last_gphi is None or last_gpsi is None:
                                raise RuntimeError("Expected manifold gradients for safe step.")
                            lr_m0 = manifold_lr if manifold_lr is not None else base_lrs[0]
                            lr_m = lr_m0 * scale
                            with torch.no_grad():
                                ok = _apply_qr_update(params.Phi, params.Psi, last_gphi, last_gpsi, lr_m)
                            if not ok:
                                continue
                    ...
                    if is_nitrom and manifold_retraction == "qr":
                        if last_gphi is None or last_gpsi is None:
                            raise RuntimeError("Expected manifold gradients for QR update.")
                        lr_m = manifold_lr if manifold_lr is not None else optimizer.param_groups[0]["lr"]
                        with torch.no_grad():
                            _apply_qr_update(params.Phi, params.Psi, last_gphi, last_gpsi, lr_m)

                    optimizer.step()
                    loss_for_log = float(_eval_loss().detach().item())
                    lr_tensors_used = float(optimizer.param_groups[0]["lr"])
                    lr_manifold_used = float(manifold_lr) if manifold_lr is not None else lr_tensors_used
