...
def compute_indices(c_ls=[], c=0, idx=5, r=5, order=0):
    ...
    for i in range(idx):
        ...
        if order == 0:
            ...
            c_ls = compute_indices(c_ls, ci, i + 1, r, order - 1)
    return c_ls
...
def assemble_Y(pool, Phi):
    ...
    r = Phi.shape[-1]
    Y = torch.zeros(
        (r, pool.n_traj * pool.n_snapshots), device=Phi.device, dtype=Phi.dtype
    )
    for i in range(pool.n_traj):
...
def assemble_W(pool):
    ...
    W = torch.zeros(
        pool.n_traj * pool.n_snapshots, device=weights.device, dtype=weights.dtype
    )
    for i in range(pool.n_traj):
        W[i * pool.n_snapshots : (i + 1) * pool.n_snapshots] = 1.0 / (
            weights[i] * pool.n_traj
        )
...
def assemble_Z(pool, Phi, poly_comp):
    ...
    for count, p in enumerate(poly_comp):
        ...
        Z_ = torch.zeros(
            (rp, pool.n_traj * pool.n_snapshots), device=Phi.device, dtype=Phi.dtype
        )
...
def solve_least_squares_problem(pool, Z, Y, W, P):
    ...
    return Y @ W @ Z.T @ u[:, idces] @ torch.diag(1.0 / s[idces]) @ (u[:, idces]).T
