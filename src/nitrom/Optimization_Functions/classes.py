...
class pool:
    def __init__(
        ...
        **kwargs,
        ...
        self.time = torch.tensor(
            np.load(fname_time), device=self.device, dtype=self.dtype
        )
    ...
    def load_time_derivatives(self, kwargs):
        ...
        if fname_deriv is not None:
            ...
            if self.my_n_traj > 0:
                dX = [
                    np.load(self.fnames_deriv[k]) for k in range(len(self.fnames_deriv))
                ]
...
class optimization_objects:
    def __init__(
        ...
        **kwargs,
        ...
        if len(local_which_trajs) > 0:
            ...
            self.X = torch.zeros(
                (0, pool.N, len(which_times)), device=pool.device, dtype=pool.dtype
            )
            self.dX = torch.zeros(
                (0, pool.N, len(which_times)), device=pool.device, dtype=pool.dtype
            )
    ...
    def evaluate_rom_rhs(self, t, z, u, *operators, **kwargs):
        ...
        f = (
            ...
            else torch.zeros(n, device=z.device, dtype=z.dtype)
        )
