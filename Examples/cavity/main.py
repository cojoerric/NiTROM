...
which_times = np.arange(0,pool.n_snapshots//2,1)
leggauss_deg = 5
nsave_rom = 15
...
if run_opinf_gs:
    ...
    optimizer = optimizers.ConjugateGradient(max_iterations=2500,min_step_size=1e-20,max_time=3600,line_searcher=line_searcher,verbosity=verb,log_verbosity=1)

    weights = pool.weights.copy()
    pool.weights *= pool.n_traj*pool.n_snapshots

    lam = np.logspace(-6,-4,num=20)
    cost_oi_gs = []
    for count, l in enumerate(lam):
