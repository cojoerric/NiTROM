...
def save_uncover_line_slides(
    ...
    *,
    ...
    for y, label, color, linestyle, linewidth in zip(
        y_series, labels, colors, linestyles, linewidths
    ):
        line = ax.plot(
    ...
    if ylim is not None:
        ...
        if log_y and torch.any(all_y <= 0):
            raise ValueError(
                "All y-values must be positive when 'log_y=True' and 'ylim' is not provided."
            )
    ...
    for i, line in enumerate(lines, 1):
        ...
        if show_legend:
            ...
            if fixed_legend_size:
                ...
                legend = ax.legend(
                    legend_handles, legend_labels, loc=legend_loc, handlelength=2.0
                )
                for txt, alpha in zip(
                    legend.get_texts(), [1.0] * i + [0.0] * (n_lines - i)
                ):
                    txt.set_alpha(alpha)
            else:
        ...
        fig.canvas.draw()
        fig.savefig(
            f"{output_prefix}_{i}.png", dpi=600, bbox_inches="tight", pad_inches=0.03
        )
