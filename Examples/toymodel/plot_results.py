...
for k in range(len(betas)):
    ...
    sol_nit_gs_r = integrators.my_rk4_adaptive(opt_obj.evaluate_rom_rhs, t_eval, z0, args=(psi_nit_gs.T@u,) + tensors_nit_gs)
    sol_nit_gs = phi_nit_gs @ sol_nit_gs_r
    error_nit_gs += torch.norm(C @ (sol_nit_gs - sol), dim=0)**2 / weight / len(betas)

fig, ax = make_figure()
ax.semilogy(t_eval.cpu()[:idx_10], error_pod.cpu()[:idx_10], label='POD-Gal.', color=cPOD, linestyle=lPOD)
ax.semilogy(t_eval.cpu()[:idx_10], error_oi.cpu()[:idx_10], label='OpInf', color=cOI, linestyle=lOI)
ax.semilogy(t_eval.cpu()[:idx_10], error_oi_gs.cpu()[:idx_10], label='GasOpInf', color=cOI_gs, linestyle=lOI_gs)
ax.semilogy(t_eval.cpu()[:idx_10], error_nit.cpu()[:idx_10], label='NiTROM', color=cNIT, linestyle=lNIT)
ax.semilogy(t_eval.cpu()[:idx_10], error_nit_gs.cpu()[:idx_10], label='GasNiTROM', color=cNIT_gs, linestyle=lNIT_gs)
...
ax.semilogy(t_eval.cpu(), error_oi_gs.cpu(), label='GasOpInf', color=cOI_gs, linestyle=lOI_gs)
ax.semilogy(t_eval.cpu(), error_nit.cpu(), label='NiTROM', color=cNIT, linestyle=lNIT)
ax.semilogy(t_eval.cpu(), error_nit_gs.cpu(), label='GasNiTROM', color=cNIT_gs, linestyle=lNIT_gs)
...
if slides:
    save_uncover_line_slides(
        t_eval.cpu(),
        [error_pod.cpu(), error_oi.cpu(), error_nit.cpu(), error_oi_gs.cpu(), error_nit_gs.cpu()],
...
sol_fom = integrators.my_rk4_adaptive(fom.evaluate_fom_dynamics, tk, torch.zeros(3, device=device, dtype=dtype), args=(fu,))

fu = Interp1D(tk, torch.outer((psi_pod.T@B).flatten(), uk), extrapolate=True)
sol_pod = phi_pod @ integrators.my_rk4_adaptive(opt_obj.evaluate_rom_rhs, tk, torch.zeros(r, device=device, dtype=dtype), args=(fu,) + tensors_pod)

fu = Interp1D(tk, torch.outer((psi_pod.T@B).flatten(), uk), extrapolate=True)
sol_oi = phi_pod @ integrators.my_rk4_adaptive(opt_obj.evaluate_rom_rhs, tk, torch.zeros(r, device=device, dtype=dtype), args=(fu,) + tensors_oi)
sol_oi_gs = phi_pod @ integrators.my_rk4_adaptive(opt_obj.evaluate_rom_rhs, tk, torch.zeros(r, device=device, dtype=dtype), args=(fu,) + tensors_oi_gs)

fu = Interp1D(tk, torch.outer((psi_nit.T@B).flatten(), uk), extrapolate=True)
sol_nit = phi_nit @ integrators.my_rk4_adaptive(opt_obj.evaluate_rom_rhs, tk, torch.zeros(r, device=device, dtype=dtype), args=(fu,) + tensors_nit)

fu = Interp1D(tk, torch.outer((psi_nit_gs.T@B).flatten(), uk), extrapolate=True)
sol_nit_gs = phi_nit_gs @ integrators.my_rk4_adaptive(opt_obj.evaluate_rom_rhs, tk, torch.zeros(r, device=device, dtype=dtype), args=(fu,) + tensors_nit_gs)

fig, ax = make_figure()
ax.plot(t_eval[:idx_10], fom.compute_output(sol_fom)[0,][:idx_10].cpu(), color='k', linewidth=2.4, label='FOM')
ax.plot(t_eval[:idx_10], fom.compute_output(sol_pod)[0,][:idx_10].cpu(), color=cPOD, linestyle=lPOD, label='POD-Gal.')
ax.plot(t_eval[:idx_10], fom.compute_output(sol_nit)[0,][:idx_10].cpu(), color=cNIT, linestyle=lNIT, label='NiTROM')
ax.plot(t_eval[:idx_10], fom.compute_output(sol_nit_gs)[0,][:idx_10].cpu(), color=cNIT_gs, linestyle=lNIT_gs, label='GasNiTROM')
ax.plot(t_eval[:idx_10], fom.compute_output(sol_oi)[0,][:idx_10].cpu(), color=cOI, linestyle=lOI, label='OpInf')
ax.plot(t_eval[:idx_10], fom.compute_output(sol_oi_gs)[0,][:idx_10].cpu(), color=cOI_gs, linestyle=lOI_gs, label='GasOpInf')
style_axes(ax, xlabel='Time $t$', ylabel='$y(t)$', xlim=(0.0, 10.0))
save_figure(fig, 'response_toymodel_low_10')

fig, ax = make_figure(wide=True)
ax.plot(t_eval, fom.compute_output(sol_fom)[0,].cpu(), color='k', linewidth=2.4, label='FOM')
ax.plot(t_eval, fom.compute_output(sol_pod)[0,].cpu(), color=cPOD, linestyle=lPOD, label='POD-Gal.')
ax.plot(t_eval, fom.compute_output(sol_oi)[0,].cpu(), color=cOI, linestyle=lOI, label='OpInf')
ax.plot(t_eval, fom.compute_output(sol_nit)[0,].cpu(), color=cNIT, linestyle=lNIT, label='NiTROM')
ax.plot(t_eval, fom.compute_output(sol_oi_gs)[0,].cpu(), color=cOI_gs, linestyle=lOI_gs, label='GasOpInf')
ax.plot(t_eval, fom.compute_output(sol_nit_gs)[0,].cpu(), color=cNIT_gs, linestyle=lNIT_gs, label='GasNiTROM')
